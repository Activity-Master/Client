	...
	default Uni<IRelationshipValue<J, IResourceItemType<?, ?>, ?>> addResourceItemTypes(Mutiny.Session session, String resourceType, String value, String classificationName, ISystems<?, ?> system, UUID... identityToken)
	{
		...
		.chain(table -> {
				return table.createDefaultSecurity(session, system, identityToken)
						.replaceWith((IRelationshipValue<J, IResourceItemType<?, ?>, ?>) table);
			});
	}
						...
						return session.fetch(system)
									...
									.chain(() -> {
										...
										return flagService.getActiveFlag(session, originalEnterprise, identityToken)
												.map(activeFlag -> {
													...
													configureResourceItemTypeLinkValue(newTableForClassification, (J) this, resourceItemType, classification, value, systemEnterprise);
							...
							.chain(newTable -> {
								return newTable.createDefaultSecurity(session, originalSystem, identityToken)
										.replaceWith((IRelationshipValue<J, IResourceItemType<?, ?>, ?>) newTable);
							})))));
			});
		});
	}
