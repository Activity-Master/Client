...
@Log4j2
public class InvolvedPartyTypeDeserializer extends JsonDeserializer<IInvolvedPartyType<?, ?>>
{
    @Override
    public IInvolvedPartyType<?, ?> deserialize(JsonParser p, DeserializationContext ctxt) throws IOException, JsonProcessingException
    {
        ...
        if (!value.startsWith("{"))
        {
            ...
            catch (Exception e)
            {
                ...
                throw new IOException("Failed to fetch IInvolvedPartyType from DB", e);
            }
        }
        else
        {
            ...
            catch (Exception e)
            {
                ...
                throw new IOException("Failed to deserialize embedded IInvolvedPartyType JSON", e);
            }
        }
    }
}
