...
public interface IClassificationService<J extends IClassificationService<J>> {
    ...
    default Uni<IClassification<?, ?>> create(Mutiny.StatelessSession session,
                                              ...
                                              UUID... identityToken) {
        if (parent == null) {
            return create(session, name.toString(), name.toString(), concept, system, 0, identityToken);
        }
        return create(session, parent.toString(), system, identityToken).chain(classification -> create(session,
                                                                                                        name.toString(),
                                                                                                        name.toString(),
     ...
     * provisioned in parallel across independent stateless sessions.
     */
    Uni<IClassification<?, ?>> createScopeRestricted(Mutiny.StatelessSession session,
                                                     String name,
                                                     String description,
    ...
    default Uni<IClassification<?, ?>> create(Mutiny.StatelessSession session,
        ...
        return create(session, parent, parent, system, identityToken).chain(parentClassification -> create(session,
                                                                                                           name.toString(),
                                                                                                           name.toString(),
                                                                                                           system,
    ...
    default Uni<IClassification<?, ?>> create(Mutiny.StatelessSession session,
        ...
        return create(session,
                      ...
                      parent.toString(),
                      system,
                      identityToken).chain(parentClassification -> create(session,
                                                                          name.toString(),
                                                                          name.toString(),
                                                                          system,
                                                                          parentClassification,
     ...
     * {@link Mutiny.StatelessSession}. Shares the same
     ...
     */
    default Uni<UUID> resolveClassificationIdByName(Mutiny.StatelessSession session,
                                                    UUID enterpriseId,
                                                    UUID systemId,
                                                    UUID conceptId,
