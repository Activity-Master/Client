...
@Log4j2
public class InvolvedPartyIdentificationTypeDeserializer extends ValueDeserializer<IInvolvedPartyIdentificationType<?, ?>>
{
    @Override
    public IInvolvedPartyIdentificationType<?, ?> deserialize(JsonParser p, DeserializationContext ctxt) throws JacksonException
    {
        ...
        if (!value.startsWith("{"))
        {
            ...
            catch (Exception e)
            {
                ...
                throw new RuntimeException("Failed to fetch IInvolvedPartyIdentificationType from DB", e);
            }
        }
        else
        {
            ...
            catch (Exception e)
            {
                ...
                throw new RuntimeException("Failed to deserialize embedded IInvolvedPartyIdentificationType JSON", e);
            }
        }
    }
}
