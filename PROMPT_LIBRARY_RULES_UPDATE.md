...
- Library name: Activity Master Client
- Current/new version: 3.0.0-SNAPSHOT
- Repository URL / path: c:\Java\DevSuite\ActivityMaster\client
- Rules Repository Location: rules/
- Short description: Reactive client library for Activity Master SPI (EntityAssist builders, systems/tokens/classifications) backed by Hibernate Reactive and GuicedEE.
- Type:
...
- MCP servers to register (Mermaid MCP required; add others as needed): Mermaid MCP (diagrams) `https://mcp.mermaidchart.com/mcp` type `http`. Keep secrets out of the repo; reference env var names instead.
...
- Component/topic areas (list): Activity Master SPI services; EntityAssist query builders; system bootstrap and token caching; classification data concepts and row-level security; system registration/updates; testing bootstraps.
